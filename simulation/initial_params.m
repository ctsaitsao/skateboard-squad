...
% Bottom Link Parameters
    ...
    params.bottomLinkYCoM = .107; % m, taken wrt the joint with the skateboard (not equal to the bottomLinkYCoM of derive_equations, which is wrt global frame), from SolidWorks
...
% Top Link Parameters
    ...
    params.topLinkYCoM = 0.2; % m, taken wrt the joint with the skateboard (not equal to the bottomLinkYCoM of derive_equations, which is wrt global frame), from SolidWorks
    
    params.sim.dt = 0.001;
    dt = params.sim.dt;
    params.sim.delay = 0.5*params.sim.dt;  % commmunication and computation delay
    params.sim.tfinal = .2;
    ...
    params.sim.constraints = logical([1, 0]); % left and right
